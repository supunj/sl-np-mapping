...
# ogr2ogr -f SQLite -dsco SPATIALITE=YES -lco GEOMETRY_NAME=geom $base_dir/tmp/$np-map-extent.db $base_dir/poly/$np.geojson
...
# ogr2ogr -f GeoJSON $base_dir/tmp/$np-shaded-relief-map-extent.geojson $base_dir/tmp/$np-map-extent.db $np

# Full extent of the map polygon
map_extent_poly=$base_dir/poly/$np.geojson
...
gdal_translate -a_srs EPSG:4326 -a_ullr $1 $4 $3 $2 $base_dir/tmp/$np-srtm-combined-transparent.tiff $base_dir/tmp/$np-srtm-combined-transparent-geo-referenced.tiff

# Cut out the exact park boundary polygon
gdalwarp -overwrite -cutline $map_extent_poly -crop_to_cutline -dstalpha -cblend 0 $base_dir/tmp/$np-srtm-combined-transparent-geo-referenced.tiff $base_dir/var/$np-srtm-combined-transparent-geo-referenced-cropped.tiff
