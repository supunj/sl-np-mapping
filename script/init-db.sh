...
"$gdal_contour_bin" -a elev -i 10 -f SQLite -dsco SPATIALITE=YES -lco GEOMETRY_NAME=geom -nln contours $base_dir/var/$np-srtm.tiff $base_dir/db/$np.db

# then insert the geometries from the OSM file
"$ogr2ogr_bin" -update -f SQLite -dsco SPATIALITE=YES -lco GEOMETRY_NAME=geom $base_dir/db/$np.db $base_dir/var/$np-cleansed-merged.osm

# Insert the coastline
"$ogr2ogr_bin" -update -f SQLite -dsco SPATIALITE=YES -lco GEOMETRY_NAME=geom -nln sl $base_dir/db/$np.db $base_dir/var/sl-coastline.osm multipolygons

# Insert admin features
"$ogr2ogr_bin" -update -f SQLite -dsco SPATIALITE=YES -lco GEOMETRY_NAME=geom -nln sl_admin $base_dir/db/$np.db $base_dir/var/sl-admin.osm lines
