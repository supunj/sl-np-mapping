        ...
        if layer.isValid():
            if layer.geometryType() == Qgis.GeometryType.Polygon:
                ...
                symbol = QgsLineSymbol.createSimple(
                    {"color": table_layer[2], "width": table_layer[3]}
                )
            elif layer.geometryType() == Qgis.GeometryType.Point:
                symbol = QgsMarkerSymbol.createSimple(
                    {"name": "circle", "color": table_layer[2], "size": table_layer[3]}
                )
...
def calculatePageSize(project, extent, scale):
    ...
    extent_height = da.convertLengthMeasurement(
        extent.height(), Qgis.DistanceUnit.Meters
    )
...
def createLayout(project, scale, layout_name):
    ...
    page.setPageSize(
        QgsLayoutSize(page_width_mm, page_height_mm, Qgis.LayoutUnit.Millimeters)
    )
...
def main():
    ...
    if format == "svg":
        result = exportToSVG(
            project.layoutManager().layoutByName(layout_name), output_file_path
        )
    elif format == "png":
        result = exportToPNG(
            project.layoutManager().layoutByName(layout_name), output_file_path
